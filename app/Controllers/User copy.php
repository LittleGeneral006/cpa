...
class User extends BaseController
{
    ...
    public function proses_login()
    {
        ...
        $resUser = $users->getSessionUser($username);
                ...
                if (password_verify($password, $data_user->password_user)) {
                    ...
                    if ($resUser > 0) {
                        if ($cek_username > 0) {
                            ...
                            $this->db->table('tb_user')->where('username_user', $username)->update($data);
                            session()->setFlashdata('message', '<center>Login Berhasil!<br>User sedang digunakan di perangkat lain!</center>');
                            $dataSesi = [
                                'session_id' => session_id(),
                                'created_at' => $this->now,
                            ];
                            $users->updateSession(session()->get('username'), $dataSesi);
                        ...
                        if ($cek_username > 0) {
                            ...
                            $dataSesi = [
                                'session_id' => session_id(),
                                'session_user' => session()->get('username_user'),
                                'created_at' => $this->now,
                            ];
                            $this->UsersModel->setSession($dataSesi);
                            $data = [
                                'last_login_user' => gmdate("Y-m-d H:i:s", time() + 60 * 60 * 8),
                                // 'last_login_user' => $this->now,
                                'sha_user' => sha1('SHA' . gmdate("Y-m-d H:i:s", time() + 60 * 60 * 8)),
