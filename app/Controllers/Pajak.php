...
class Pajak extends BaseController
{
    ...
    // }
    public function v_pajak()
    {

        $hasil = $this->hak_akses();
        if ($hasil == true) {
            $data['title'] = 'Master Pajak';
            return view('backend/v_pajak', $data);
        } else {
    ...
    public function tabel_pajak()
    {
        $sQuery1 = "SELECT * FROM tb_pajak ";
        $sQuery2 = "SELECT COUNT(ID) AS TOTFIL FROM tb_pajak ";
        $sQuery3 = "SELECT COUNT(ID) AS TOT FROM tb_pajak ";
        $sIndexColumn = "ID";
        ...
        $sQuery = $sQuery1 . $sWhere . $sOrder  . $sLimit;
        ...
        foreach ($rResult as $aRow) {
            ...
            $row = array();
            $row[] = $aRow->nama_pajak; //0
            $row[] = $aRow->persentase_pajak; //1
            ...
            //4
            $row[] = '<button title="Edit" id="edit_pajak" class="btn btn-primary btn-sm  edit_pajak"><i class="fa fa-pencil" aria-hidden="true"></i></button>' . '<button title="Detail" id="detail_unit" class="btn btn-warning btn-sm"><i class="fa fa-eye" aria-hidden="true"></i></button>'; //6
            $row[] = $aRow->ID; //5
            $output['aaData'][] = $row;
            // $i++;
        }
    ...
    //     }
    //     //4
    //     $row[] = '<button title="Edit" id="edit_pajak" class="btn btn-info btn-sm"><i class="fa fa-pencil" aria-hidden="true"></i></button>' . '<button title="Detail" id="detail_unit" class="btn btn-success btn-sm"><i class="fa fa-eye" aria-hidden="true"></i></button>'; //6
    ...
    {
        $aRow = $this->db->query("SELECT * FROM tb_pajak where ID = '" . $id . "' ")->getRow();
        //foreach ($rResult as $aRow) {
        $row = array();
        $row['pajak'] = $aRow; //0
