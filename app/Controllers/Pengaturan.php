...
class Pengaturan extends BaseController
{
    ...
    public function index()
    {
        ...
        if ($hasil == true) {
            ...
            return view('backend/v_pengaturan', $data);
        } else {
    ...
    public function edit_pengaturan()
    {
        ...
        if (!$this->validate([
            ...
            'singkatan_web_edit' => [
                'rules' => 'required',
                'errors' => [
                    'required' => 'Singkatan web harus diisi',
            ...
            $data = [
                'singkatan_web' => $this->request->getPost('singkatan_web_edit'),
                'judul_web' => $this->request->getPost('judul_web_edit'),
                'copyright' => $this->request->getPost('copyright_edit'),
                'warna_tema' => $this->request->getPost('warna_tema_edit'),
            ];
            //pengecekan kd info tidak boleh sama sebelum insert
            $id = $this->request->getPost('id_edit');
            $cek_id = $this->db->query("SELECT id from tb_pengaturan where id = '" . $id . "' ")->getNumRows();
            if ($cek_id > 0) {
                $this->db->table('tb_pengaturan')->where('id', $id)->update($data);
                $pengaruh = $this->db->affectedRows();
                echo json_encode($pengaruh);
            } else {
                echo 'Setup Web Gagal Karena Data Tidak Ditemukan';
            }
        }
    }
}
